class CreateDraftJob < ApplicationJob
  ...
  def perform(questionnaire)
    ...
    Rails.logger.info "Created Cumulus provider for #{questionnaire.uid}: #{cumulus_provider_response}"

    cumulus_collection_response = CumulusApi.create_cumulus_collection(collection_info)
    Rails.logger.info "Created Cumulus collection for #{questionnaire.uid}: #{cumulus_collection_response}"
